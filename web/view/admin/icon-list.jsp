            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <strong class="card-title">Danh sách icon</strong>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 col-xs-6">
                                <a data-toggle="tooltip" title="Thêm mới icon" href="<%=request.getContextPath()%>/admin/create-icon.htm"><button type="button" class="btn btn-success btn-sm mr-2"><i class="menu-icon ti-plus"></i>&nbsp; Thêm mới</button></a>
                            </div>
                        </div>
                        <div class="card-body">
                            <table id="myTable" class="table table-striped table-bordered">
                                ...
                                <tbody>
                                    <c:forEach items="${listIcon}" var="icon">
                                        <tr>
                                            <td>
                                                <img src="${pageContext.request.contextPath}${icon.iconImage}" width="50" class="img-circle">
                                            </td>
                                            <td>
                                                <c:if test="${icon.iconStatus == 1}">
                                                    <span style="padding: 4px 9px 5px 9px;" class="badge badge-pill badge-success">Kích hoạt</span>
                                                </c:if>
                                                <c:if test="${icon.iconStatus == 0}">
                                                    <span style="padding: 4px 9px 5px 9px;" class="badge badge-pill badge-secondary">Tạm ẩn</span>
                                                </c:if>
                                            </td>
                                            <td>
                                                <a class="mr-3" href="update-banner.htm?bannerId=${icon.iconId}" data-toggle="tooltip" data-placement="top-center" title="Chỉnh Sửa"><span class="ti-pencil"></span></a>
                                                <a data-id="${icon.iconId}" class="btn-delete" ><span data-toggle="tooltip" data-placement="top" title="Xóa" class="ti-trash"></span></a>
                                            </td>
                                        </tr>
                                    </c:forEach>
                                </tbody>
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        if (res === "success") {
                            ...
                            window.location.replace("icons.htm");
                        }
                    }
                });
            });
