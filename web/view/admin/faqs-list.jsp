            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <strong class="card-title">Danh mục FAQs</strong>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 col-xs-6">
                                <a data-toggle="tooltip" title="Thêm mới banner" href="<%=request.getContextPath()%>/admin/create-faqs.htm"><button type="button" class="btn btn-success btn-sm mr-2"><i class="menu-icon ti-plus"></i>&nbsp; Thêm mới</button></a>
                            </div>
                        </div>
                        <table id="myTable" class="table table-striped table-bordered">
                            ...
                            <tbody>
                                <c:forEach items="${listFaq}" var="faq">
                                    <tr>
                                        <td>${faq.fastQuestion}</td>
                                        <td>${faq.fastAnswer}</td>
                                        <td>
                                            <c:if test="${faq.faqstatus == 1}">
                                                <label style="border-radius: 20px;padding: 0px;width: 50%" class="btn btn-success btn-sm">Kích hoạt</label>
                                            </c:if>
                                            <c:if test="${faq.faqstatus == 0}">
                                                <label style="border-radius: 20px;padding: 0px;width: 50%" class="btn btn-secondary btn-sm">Tạm khoá</label>
                                            </c:if>
                                        </td>
                                        <td>
                                            <a class="mr-3" href="update-faqs.htm?faqid=${faq.faqid}" data-toggle="tooltip" data-placement="top-center" title="Chỉnh Sửa"><span class="ti-pencil"></span></a>
                                            <a data-id="${faq.faqid}" class="btn-delete" href="" data-toggle="modal" data-target="#staticModal"><span data-toggle="tooltip" data-placement="top" title="Xóa" class="ti-trash"></span></a>
                                        </td>
                                    </tr>
                                </c:forEach>
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        if (res === "success") {
                            ...
                            window.location.replace("faqs.htm");
                        }
                    }
                });
            });
