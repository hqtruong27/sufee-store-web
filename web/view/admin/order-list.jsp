                                ...
                                <tbody>
                                    <c:forEach items="${listOrders}" var="order">
                                        <tr>
                                            ...
                                            <td>${order.email}</td>
                                            <td>${order.phone}</td>
                                            <td><fmt:formatDate pattern="dd/MM/yyy" value="${order.createDate}"/></td>
                                            <td><fmt:formatNumber value="${order.orderTotalAmount}"/> ₫</td>
                                            <td><c:if test="${order.orderStatus == 0}">
                                                    <label style="border-radius: 20px;padding: 0px;width: 100%" class="btn btn-warning btn-sm">Đang chờ xác nhận</label>
                                                </c:if>
                                                <c:if test="${order.orderStatus == 1}">
                                                    <label style="border-radius: 20px;padding: 0px;width: 100%" class="btn btn-success btn-sm">Đã giao hàng</label>
                                                </c:if>
                                                <c:if test="${order.orderStatus == -1}">
                                                    <label style="border-radius: 20px;padding: 0px;width: 100%" class="btn btn-danger btn-sm">Đơn đã hủy</label>
                                                </c:if>
                                            </td>
                                            <td style="text-align: center">
                                                <a class="mr-3" href="orders-detail.htm?orderId=${order.orderId}" data-toggle="tooltip" data-placement="bottom" title="Chi tiết"><span class="ti-eye"></span></a>
                                            </td>
                                        </tr>
                                    </c:forEach>
                                </tbody>
