            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <p>
                                <a href="<%=request.getContextPath()%>/admin/create-product.htm"><button type="button" class="btn btn-success btn-sm mr-2"><i class="menu-icon ti-plus"></i>&nbsp; Thêm mới sản phẩm</button></a>
                                <a href="<%=request.getContextPath()%>/admin/brand.htm"><button type="button" class="btn btn-primary btn-sm mr-2"><i class="menu-icon ti-layout-tab-window"></i>&nbsp; Hãng sản xuất</button></a>
                                <a href="<%=request.getContextPath()%>/admin/categories.htm"><button type="button" class="btn btn-danger btn-sm mr-2"><i class="menu-icon ti-layout-tab-window"></i>&nbsp; Danh mục sản phẩm</button></a>
                            </p>
                            <table id="myTable" id="bootstrap-data-table-export" class="table table-striped table-bordered">
                                <thead>
                                    <tr>
                                        <th>Tên sản phẩm</th>
                                        <th>Mã hiệu</th>
                                ...
                                <tbody>
                                    <c:forEach items="${listProduct}" var="p">
                                        <tr id="rows_${p.productId}">
                                            <td>
                                                <img src="${p.featureImage}" alt="NoImg" width="50" class="img-circle">
                                                <span>
                                                    <c:if test="${fn:length(p.productName) > 30}">
                                                        ${fn:substring(p.productName, 0, 30)} ...
                                                    </c:if>
                                                    <c:if test="${fn:length(p.productName) <= 30}">
                                                        ${p.productName}
                                                    </c:if>
                                            ...
                                            <c:if test="${not empty listCategories}">
                                                <c:forEach items="${listCategories}" var="c">
                                                    <c:if test="${c.categoryId == p.categories.categoryId}">
                                            ...
                                            <td>${p.productSale}%</td>
                                            <td>${p.warranty} năm</td>
                                            <td>
                                                <c:if test="${p.productStatus == 1}">
                                                    <span style="padding: 4px 9px 5px 9px;" class="badge badge-pill badge-success">Còn hàng</span>
                                                </c:if>
                                                <c:if test="${p.productStatus == 2}">
                                                    <span style="padding: 4px 9px 5px 9px;" class="badge badge-pill badge-warning">Hết hàng</span>
                                                </c:if>
                                                <c:if test="${p.productStatus == 0}">
                                                    <span style="padding: 4px 9px 5px 9px;" class="badge badge-pill badge-dark">Tạm ẩn</span>
                                                </c:if>
                                            </td>
                                            <td>
                                                <a class="mr-3" href="<%=request.getContextPath()%>/admin/update-product.htm?productId=${p.productId}" data-toggle="tooltip" data-placement="top-center" title="Chỉnh Sửa"><span class="ti-pencil"></span></a>
                                                <a class="btn-delete" href="" data-id="${p.productId}"><span data-toggle="tooltip" data-placement="top" title="Xóa" class="ti-trash"></span></a>
                                            </td>
                                        </tr>
                                    </c:forEach>
                                </tbody>
