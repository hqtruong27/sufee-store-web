                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <strong>Chỉnh sửa danh mục sản phẩm</strong> 
                        </div>
                        <div class="card-body card-block">
                            <f:form action="update-category.htm" commandName="category" method="POST" class="form-horizontal">    
                                <f:input path="categoryId" type="hidden" id="categoryId" class="form-control"/>
                                <div class="row form-group">
                                    <div class="col col-md-3"><label for="text-input" class=" form-control-label">Tên danh mục</label></div>
                                    <div class="col-12 col-md-9">
                                        <f:input path="categoryName" type="text" id="categoryName" class="form-control"/>
                                ...
                                <div class="row form-group">
                                    <div class="col col-md-3"><label for="text-input" class=" form-control-label">Độ ưu tiên</label></div>
                                    <div class="col-12 col-md-9">
                                        <input value="${category.categoryPiority}" name="categoryPiority" type="text" id="categoryPiority" class="form-control" required oninvalid="this.setCustomValidity('Vui lòng chọn vị trí sắp xếp')" oninput="this.setCustomValidity('')"/>
                                        <c:if test="${not empty errorcategorypority}"><small class="form-text text-danger">${errorcategorypority}</small></c:if>
                                        </div>
                                    </div>
                                    <div class="row form-group">
                                        ...
                                        <c:choose>
                                            ...
                                            <c:when test="${category.categoryStatus == 0}">
                                                <input  id="categoryStatus" class="" type="radio" name="categoryStatus" value="1" />
                                                <label for="categoryStatus">kích hoạt</label>
                                                <input  id="deActive" class="" type="radio" name="categoryStatus" value="0" checked=""/>
                                                <label for="deActive">Tạm ẩn</label>
                                            </c:when>
                                            <c:otherwise>
                                                <!--No comment sir...-->
                                    ...
                                    <div class="row form-group">
                                        ...
                                        <c:choose>
                                            <c:when test="${category.parentId == 0}">
                                                <a href="<%=request.getContextPath()%>/admin/categories.htm" type="reset" class="btn btn-danger btn-sm">
                                                    <i class="ti-back-left"></i> Trở lại
                                                </a>
                                            </c:when>
                                            <c:when test="${category.parentId != 0}">
                                                <a href="<%=request.getContextPath()%>/admin/categories-parent.htm"  type="reset" class="btn btn-danger btn-sm">
                                                    <i class="ti-back-left"></i> Trở lại
                                                </a>
                                            </c:when>
                                        </c:choose>
